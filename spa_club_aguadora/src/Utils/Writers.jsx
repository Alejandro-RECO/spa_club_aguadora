export const MEDIA_PATHS = {
   NAV_LINKS: {
      HOME: "/",
      RESTAURANT: "restaurante",
      ABOUT_US: "quienessomos",
   },
   HEADER: {
      IMAGEN: "/logo.png",
      ELEMETS_TEXT: {
         HOME: "inicio",
         RESTAURANT: "restaurante y eventos",
         ABOUT_US: "quienes somos",
      },
      NETWORKS: {
         FACEBOOK: "https://www.facebook.com/profile.php?id=61557136467588",
         INSTAGRAM: "https://www.instagram.com/club_la_aguadora/",
         TWITER: "https://x.com",
      },
   },
   HOME: {
      CAROUSEL: [
         {
            IMAGE: "https://www.clublaaguadora.com/wp-content/uploads/2022/09/Salon01-1.bmp",
            TITLE: "Tus mejores planes, ¡Vívelos en tu Club La Aguadora!",
         },
         {
            IMAGE: "https://www.clublaaguadora.com/wp-content/uploads/2022/09/La_Aguadora_Escultura_80-1.jpg",
            TITLE: "¡Bienvenidos a la Corporación Club Social La Aguadora!",
         },
         {
            IMAGE: "https://www.clublaaguadora.com/wp-content/uploads/2022/09/Sauna-turco01-1-scaled.jpg",
            TITLE: "Descansa, nuestras zonas húmedas están a tu disposición.",
         },
         {
            IMAGE: "https://www.clublaaguadora.com/wp-content/uploads/2022/09/Salon02.bmp",
            TITLE: "Deleitate y comparte en nuestros salones y restaurante",
         },
      ],
      INFORMATION: [
         {
            TITLE: "pagos al día",
            DESCRIPTION:
               "Te invitamos a estar al día en tus compromisos de pago, con tu aporte hacemos que nuestro Club permanezca completamente al día para tu disfrute. Gracias por tu apoyo.",
            ICON: "RiBankCard2Fill",
         },
         {
            TITLE: "reporta pagos",
            DESCRIPTION: `
          Para mayor información puedes escribirnos o llamarnos a los siguientes contactos:
          <br />
          <a class="text-primary-800 font-medium underline hover:no-underline" target="_blank" href="tel:+3013938174">Jasbleidy Rodríguez: 301 393 8174</a>
          <br />
          <a href="mailto:administracion@clublaaguadora.com">Correo: administracion@clublaaguadora.com</a>
          <br />
          <a class="text-primary-800 font-medium underline hover:no-underline" href="tel:+3012667961">Liliana Carvajal: 301 266 7961</a>
          <br />
          <a class="text-primary-800 font-medium underline hover:no-underline" href="mailto:clubaguadora@hotmail.com">Correo: clubaguadora@hotmail.com</a>
        `,
            ICON: "RiAlertFill",
         },
         {
            TITLE: "momentos compartidos",
            DESCRIPTION:
               "Nuestro operador Casa Ramirez ofrece una variedad de productos, comidas, bebidas y postres para una mejor experiencia en el Club. Te agradecemos no traer alimentos o bebidas externas. ¡Gracias por tu comprensión!",
            ICON: "RiAlarmWarningFill",
         },
         {
            TITLE: "mejor plan",
            DESCRIPTION:
               "Te invitamos a disfrutar del mejor plan en tu bolera de tu Club La Aguadora, con pistas renovadas, el más exclusivo ambiente y la mejor atención. ¡Te esperamos!",
            ICON: "GiBowlingStrike",
         },
         {
            TITLE: "Casa Ramirez",
            DESCRIPTION:
               "Organiza tus eventos con nuestros salones y el asesoramiento de Casa Ramírez. Para más información, contáctanos en gerenciacr.clubaguadora@gmail.com o WhatsApp: 320 864 6176.",
            ICON: "RiCalendarEventFill",
         },
         {
            TITLE: "Carnetización en curso",
            DESCRIPTION:
               "Estamos en plena campaña de carnetización. Actualiza tus datos y obtén tu carnet en pocos minutos. Visítanos en el Módulo Club La Aguadora, primer piso de la EAAB, Ext. 4073 o llámanos al 301 266 7961.",
            ICON: "RiPassValidFill",
         },
      ],
      NOTIAGUADORA_ED_2: {
         TITLE: "Notiaguadora",
         EDITION: "Edición 2",
         DATE: "agosto 2024",
         SUBTITLE_1: "Avances de Obras",
         DESCRIPTION_1: "Arreglo Baños La Aguadora y La Aguadita",
         CONTENT_1: [
            {
               TITLE: "Antes",
               DESCRIPTION:
                  "Los baños de nuestro querido Club se encontraban sin adecuaciones ni remodelaciones desde hace bastante tiempo, presentando problemas derivados de su uso y desgaste.",
               IMAGE_1: "./NA1.png",
               IMAGE_2: "./NA2.png",
            },
            {
               TITLE: "En Obra",
               DESCRIPTION:
                  "El 08 de mayo de 2023, Según el Acta de reunión de la Junta Directiva No. 308, se aprueba el presupuesto de $51.817.958 para la remodelación de los baños de la Bolera.",
               IMAGE_1: "./NA3.png",
               IMAGE_2: "./NA4.png",
            },
            {
               TITLE: "Ahora",
               DESCRIPTION:
                  "Ahora ya podemos contar con baterías de baños renovadas en todas las instalaciones de ¡La Aguadora, Tu Club!, en la Bolera, en los baños de los salones del piso 1 y en La Aguadita. Disfrútalos.",
               IMAGE_1: "./NA5.png",
               IMAGE_2: "./NA6.png",
            },
         ],
         SUBTITLE_2: "¿En qué anda la Junta Directiva?",
         DESCRIPTION_2: `Es prioridad para esta Junta Directiva, adelantar la actualización del Estatuto vigente y del
         Manual de Procesos y Procedimientos, esto acorde con la necesidad apremiante de
         mantenernos al día con las últimas tendencias, para lo que estamos desarrollando
         diferentes mesas de trabajo, cuyo resultado se entregará en la Asamblea General.
         El interés de la Actual Junta Directiva al actualizar las normas y procedimientos
         establecidos en el manual de procesos y procedimientos, también va orientado hacia el
         establecimiento de las reglas claras para adoptar un manejo responsable de las finanzas y
         operación general de nuestro Club.`,
         IMAGE_1: "./NA7.png",
         CONTENT_2: {
            SUBTITLE_3: "UNA EMPRESA DE TODOS PARA TODOS",
            IMAGE_1: "./NA8.jpg",
            TEXT_1:
               "La CORPORACION CLUB SOCIAL LA AGUADORA es nuestra empresa, se creó con el fin de brindar esparcimiento cultura, recreación y deporte a los asociados y sus familiares; tenemos unas instalaciones espectaculares que muy pocos tienen, por eso debemos aprovecharlas al máximo.",
            TEXT_2:
               "En el transcurso del año se han venido desarrollando variadas actividades sociales, deportivas y recreativas, actividades," +
               "en las cuales han venido participando cada vez un mayor número de personas.",
            TEXT_3:
               "La organización y la logística de todos los eventos se ha logrado a través de los comités (de Deportes, Ambiental, Social y Administrativo), la administración y los trabajadores de la Corporación a quienes les expresamos nuestros mas sinceros agradecimientos, ha sido un esfuerzo colectivo con resultados extraordinarios.",
            TEXT_4:
               "Esperamos que usted señor(a) asociado(a), familiar, invitado, visitante, participe y disfrute los eventos programados para el año 2024",
            PRESIDENT: "José Eduardo Arias Pulido",
            POST: "Presidente Junta Directiva ",
         },
         SUBTITLE_4: "y...¿Enqueseestátrabajando?",
         CONTENT_3: {
            TEXT_1:
               "Estamos repintando y delimitando las áreas comunes, parqueaderos y señalizaciones en nuestro Club. El personal de mantenimiento locativo está trabajando a toda máquina y Está quedando muuuy bonito",
            TEXT_2:
               "Nos estamos actualizando tecnológicamente, con nuevos dispositivos que nos permiten mantenernos a la vanguardia de los avances en telecomunicaciones, equipos de cómputo y seguridad informática.",
            TEXT_3:
               "Se están realizando reparaciones en la fachada y las zonas expuestas y desgastadas debido a influencia del clima. Nuestro compromiso es mantener nuestro Club hermoso siempre.",
         },
         FOOTER:
            "En ¡tu Club La Aguadora! te escuchamos, si tienes una idea de mejora, no dudes en compartirla, estamos prestos a atenderte. Escríbenos al correo lineaetica@clublaaguadora.com, agradecemos tu aporte.",
      },
   },
};
